...
#' @param list.type \code{character} type of gene symbols (ensembl,symbol)
...
#' }
#' @export
lookupGeneDescBiomart <- function(genes.list = NULL, list.type = "symbol") {
  ...
  filterString <- ""
  if (list.type == "symbol") {
    filterString <- "hgnc_symbol"
  } else {
    filterString <- "ensembl_gene_id"
  }
